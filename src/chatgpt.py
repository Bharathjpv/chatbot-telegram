...
async def clear(message: types.Message):
    ...
    clear_past()
    await message.reply(f"I have cleared the past conversation and you can start fresh")
...
async def helper(message: types.Message):
    ...
    help_command = '''
    Hello! I'am a bot created by Bharath follow these commands -
    /start - to start a conversation
    /clear - to clear the previous conversaton and context
...
async def chatgpt(message: types.Message):
    ...
    print(f'------------------------ PREV REFERENCE {reference.response}')

    reference.response = response['choices'][0]['message']['content']
    print(f'------------------------ BOT {reference.response}')
